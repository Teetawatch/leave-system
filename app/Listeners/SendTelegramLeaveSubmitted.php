...
class SendTelegramLeaveSubmitted
{
    public function handle(LeaveRequestSubmitted $event): void
    {
        try {
            ...
            // Determine who needs to approve based on current status
            $approvers = $this->getApproversForStatus($leaveRequest->status, $requester);
            ...
            Log::info('[Telegram] LeaveRequestSubmitted notification sent', [
                'leave_request_id' => $leaveRequest->id,
                'approver_count' => $approvers->count(),
            ]);
        } catch (\Throwable $e) {
            Log::error('[Telegram] LeaveRequestSubmitted error: ' . $e->getMessage());
        }
    }
    ...
    protected function getApproversForStatus(string $status, User $requester)
    {
        return match ($status) {
            ...
            'pending_deputy_director' => User::where('role', 'deputy_director')
                ->whereNotNull('telegram_chat_id')->get(),

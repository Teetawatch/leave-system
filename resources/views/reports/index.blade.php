    ...
    <div class="max-w-[95rem] mx-auto py-8 sm:px-6 lg:px-8">
        ...
        <div class="mb-8 space-y-6">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-3">
                    <button type="submit" form="filter-form" formaction="{{ route('reports.export') }}" class="inline-flex items-center px-4 py-2 bg-white border border-slate-200 rounded-xl text-slate-600 font-medium hover:bg-slate-50 hover:text-emerald-600 transition-colors shadow-sm">
                        <i data-lucide="sheet" class="w-4 h-4 mr-2"></i>
                        Export Excel
                    </button>
                    
                    <button type="button" onclick="window.print()" class="inline-flex items-center px-4 py-2 bg-white border border-slate-200 rounded-xl text-slate-600 font-medium hover:bg-slate-50 hover:text-indigo-600 transition-colors shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <!-- Total Requests -->
                <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex flex-col justify-between h-full group hover:border-indigo-100 transition-all">
                ...
                <!-- Approved -->
                 <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex flex-col justify-between h-full group hover:border-emerald-100 transition-all">
                    <div class="flex items-start justify-between">
                        <div>
                ...
                <!-- Pending -->
                 <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex flex-col justify-between h-full group hover:border-amber-100 transition-all">
                    <div class="flex items-start justify-between">
                        <div>
                 ...
                 <!-- Rejected -->
                 <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex flex-col justify-between h-full group hover:border-rose-100 transition-all">
                    <div class="flex items-start justify-between">
                        <div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6" x-data="{ showFilters: false }">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden sticky top-24">
                    ...
                    <form method="GET" action="{{ route('reports.index') }}" id="filter-form" class="p-4 space-y-5">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="space-y-2">
                                <div class="relative">
                                    ...
                                    <input type="date" name="start_date" value="{{ request('start_date') }}" class="block w-full pl-10 pr-3 py-2 border-slate-200 rounded-xl text-sm bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-indigo-500 transition-all">
                                </div>
                                <div class="relative">
                                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                        <span class="text-xs text-slate-400">ถึง</span>
                                    </div>
                                    <input type="date" name="end_date" value="{{ request('end_date') }}" class="block w-full pl-10 pr-3 py-2 border-slate-200 rounded-xl text-sm bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-indigo-500 transition-all">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="department" class="block w-full py-2 pl-3 pr-8 border-slate-200 rounded-xl text-sm bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-indigo-500 transition-all">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="leave_type_id" class="block w-full py-2 pl-3 pr-8 border-slate-200 rounded-xl text-sm bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-indigo-500 transition-all">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="status" class="block w-full py-2 pl-3 pr-8 border-slate-200 rounded-xl text-sm bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-indigo-500 transition-all">
                                <option value="">ทั้งหมด</option>
                        ...
                        <div class="pt-4 border-t border-slate-50 space-y-3">
                            <button type="submit" class="w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-md shadow-indigo-200 transition-all">
                                ค้นหา
                            </button>
                            <a href="{{ route('reports.index') }}" class="block w-full py-2.5 text-center text-slate-500 hover:text-slate-700 font-medium text-sm hover:bg-slate-50 rounded-xl transition-all">
                                ล้างค่า
                            </a>
                        </div>
                    </form>
            ...
            <div class="col-span-1 lg:col-span-3 space-y-6">
                ...
                <div class="lg:hidden">
                    <button @click="showFilters = !showFilters" class="w-full py-3 bg-white border border-slate-200 rounded-xl text-slate-600 font-bold shadow-sm flex items-center justify-center gap-2">
                        <i data-lucide="filter" class="w-4 h-4"></i>
                        ตัวกรองข้อมูล
                    </button>
                    
                    <!-- Mobile Filters -->
                    <div x-show="showFilters" class="mt-4 bg-white p-4 rounded-xl shadow-sm border border-slate-100" style="display: none;">
                        <!-- (Reuse form content for mobile if needed, or keeping it simple for now) -->
                        <p class="text-center text-slate-400 text-sm italic">ใช้ตัวกรองด้านบน (Desktop Mode) เพื่อผลลัพธ์ที่ดีที่สุด</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Top Leavers -->
                    <div class="bg-indigo-900 rounded-2xl p-6 text-white relative overflow-hidden shadow-lg shadow-indigo-200">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <span class="text-yellow-300 font-bold text-sm">{{ $leaver->total_leave_days }} วัน</span>
                                    </div>
                    ...
                    <!-- Usage Stats -->
                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex flex-col relative overflow-hidden">
                        <div class="flex items-center gap-3 mb-6 relative z-10">
                        ...
                        <div class="space-y-4 flex-1 relative z-10">
                            ...
                            <div>
                                ...
                                <div class="w-full bg-slate-100 rounded-full h-2">
                                    <div class="bg-indigo-500 h-2 rounded-full" style="width: {{ min(($type->usage_count / max($popularLeaveTypes->max('usage_count'), 1)) * 100, 100) }}%"></div>
                                </div>
                            </div>
                            @endforeach
                        </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-slate-100">
                            <thead class="bg-slate-50/50">
                                <tr>
                                    <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">พนักงาน</th>
                                    <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">ประเภท</th>
                                    <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">ช่วงเวลา</th>
                                    <th scope="col" class="px-6 py-4 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">จำนวนวัน</th>
                                    <th scope="col" class="px-6 py-4 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">สถานะ</th>
                                    <th scope="col" class="px-6 py-4 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">เอกสาร</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-50 bg-white">
                                ...
                                <tr class="hover:bg-slate-50/80 transition-colors group">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-3">
                                            @if($req->user->avatar)
                                                <img class="h-9 w-9 rounded-full object-cover shadow-sm" src="{{ asset('storage/'.$req->user->avatar) }}" alt="">
                                            @else
                                                <div class="h-9 w-9 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center font-bold text-xs">
                                                    {{ substr($req->user->rank ?? $req->user->name, 0, 1) }}
                                                </div>
                                            @endif
                                            <div>
                                                <div class="text-sm font-bold text-slate-900">{{ $req->user->rank ? $req->user->rank . ' ' : '' }}{{ $req->user->name }}</div>
                                                <div class="text-xs text-slate-500">{{ $req->user->department }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="text-sm text-slate-700 font-medium">{{ $req->leaveType->name }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        ...
                                        @endphp
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ring-1 ring-inset {{ $statusConfig['bg'] }} {{ $statusConfig['text'] }} {{ $statusConfig['border'] }}">
                                            {{ $statusConfig['label'] }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <a href="{{ route('leave-request.pdf', $req->id) }}" target="_blank" class="p-2 inline-flex text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all">

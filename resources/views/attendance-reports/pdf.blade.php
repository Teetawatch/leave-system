...
<body class="bg-slate-100 min-h-screen p-8 text-slate-800">
    ...
    <div class="no-print fixed top-6 right-6 flex gap-3 z-50">
        ...
        <a href="{{ route('attendance-reports.index') }}" class="bg-white text-slate-600 px-5 py-2.5 rounded-full shadow-lg hover:bg-slate-50 font-semibold transition-all flex items-center gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="no-print max-w-[1200px] mx-auto mb-6 bg-white rounded-xl shadow-sm border border-slate-200 p-4">
        <form action="{{ route('attendance-reports.pdf') }}" method="GET" class="flex flex-col md:flex-row gap-4 items-end">
            <div class="flex-1">
    ...
    <div class="no-print max-w-[1200px] mx-auto mb-4 flex gap-2">
        <button onclick="switchTab('student')" id="tab-student" class="tab-button active px-6 py-3 bg-white rounded-lg font-semibold text-slate-600 hover:bg-blue-50 flex items-center gap-2 shadow-sm border border-slate-200">
            <i class="fa-solid fa-graduation-cap"></i>
            นักเรียน
        </button>
        <button onclick="switchTab('employee')" id="tab-employee" class="tab-button px-6 py-3 bg-white rounded-lg font-semibold text-slate-600 hover:bg-blue-50 flex items-center gap-2 shadow-sm border border-slate-200">
            ...
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="page-break {{ $index % 2 == 0 ? 'bg-white' : 'bg-slate-50/50' }}">
                        ...
                        <td class="px-3 py-3">
                            <div class="font-bold text-slate-800">{{ $row['student']->first_name }} {{ $row['student']->last_name }}</div>
                            <div class="text-xs text-slate-400 font-mono mt-0.5">{{ $row['student']->student_code ?? '-' }}</div>
                        </td>
                        <td class="px-3 py-3 text-slate-600 text-xs max-w-[120px]">
                            <span class="bg-slate-100 px-2 py-1 rounded text-slate-500 border border-slate-200 inline-block max-w-full break-words leading-tight">
                                {{ $row['student']->course->name ?? '-' }}
                            </span>
                        </td>
                        <td class="px-3 py-3 text-center">
                            @if($row['morning'] && $row['morning']->snapshot_path)
                                <img src="https://nass.ac.th/faceattendance/storage-file?path={{ urlencode($row['morning']->snapshot_path) }}" class="w-16 h-16 object-cover rounded-lg border border-slate-200 mx-auto">
                            @else
                        ...
                        <td class="px-3 py-3 text-center font-mono font-medium">
                            @if($row['morning'])
                                <span class="{{ $row['morning_late'] ? 'text-amber-600' : 'text-slate-700' }}">
                                    {{ \Carbon\Carbon::parse($row['morning']->scan_time)->format('H:i') }}
                                </span>
                                @if($row['morning_late'])
                        ...
                        <td class="px-3 py-3 text-center">
                            @if($row['afternoon'] && $row['afternoon']->snapshot_path)
                                <img src="https://nass.ac.th/faceattendance/storage-file?path={{ urlencode($row['afternoon']->snapshot_path) }}" class="w-16 h-16 object-cover rounded-lg border border-slate-200 mx-auto">
                            @else
                                <div class="w-16 h-16 bg-slate-100 rounded-lg border border-slate-200 mx-auto flex items-center justify-center text-slate-300">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
        ...
        <table style="width: 100%; border-collapse: collapse; margin-top: 2rem;">
            <tr>
                ...
                <td style="width: 50%; text-align: center; vertical-align: bottom; padding: 0 1rem;">
                    ...
                    <div style="border-bottom: 1px dotted #94a3b8; width: 80%; margin: 0 auto; margin-bottom: 0.5rem;"></div>
    ...
    <div id="content-employee" class="tab-content print-page-break">
        <div class="max-w-[297mm] mx-auto bg-white shadow-xl rounded-none md:rounded-lg overflow-hidden print:shadow-none print:w-full print:max-w-none">
            ...
            <div class="bg-indigo-800 text-white p-8 print:bg-indigo-800 print:text-white">
                <div class="flex justify-between items-start">
                    ...
                    <div class="text-right">
                        ...
                        <p class="text-xs text-indigo-400 mt-1">พิมพ์เมื่อ: {{ \Carbon\Carbon::now()->locale('th')->isoFormat('D MMMM YYYY HH:mm') }}</p>
                    </div>
                </div>
            ...
            <div class="p-0">
                <table class="w-full text-sm text-left">
                    <thead>
                        <tr class="bg-white border-b-2 border-indigo-100 text-indigo-600 font-semibold uppercase text-xs tracking-wider">
                            <th class="px-2 py-4 w-8 text-center">ลำดับ</th>
                            <th class="px-2 py-4 w-40">ชื่อ - นามสกุล</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="page-break {{ $index % 2 == 0 ? 'bg-white' : 'bg-indigo-50/30' }}">
                            ...
                            <td class="px-3 py-3 text-slate-600 text-xs max-w-[120px]">
                                <span class="bg-indigo-100 px-2 py-1 rounded text-indigo-600 border border-indigo-200 inline-block max-w-full break-words leading-tight">
                            ...
                            <td class="px-3 py-3 text-center">
                                ...
                                @endphp
                                @if($snapshotPath)
                                    <img src="https://nass.ac.th/faceattendance/storage-file?path={{ urlencode($snapshotPath) }}" class="w-20 h-20 object-cover rounded-lg border border-slate-200 mx-auto">
                                @else

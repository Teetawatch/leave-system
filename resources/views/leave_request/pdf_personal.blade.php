...
$personalLeaveDays = $previousPersonalLeaves->sum('total_days');
    ...
    <div class="header-top">
        <div class="form-number">แบบ ๖</div>
    </div>

    <div class="form-title">ใบลากิจ</div>
    ...
    <div class="content-line">
        เรื่อง <span style="margin-right: 10px;">  ขอลากิจ</span>
    </div>
    <div class="content-line">
    ...
    <div class="content-line"> 
         ขออนุญาตลาหยุดราชการเพื่อ <span class="dotted" style="width: 410px;">{{ $leaveRequest->reason }}</span> 
    </div>
    ...
    <div class="content-line">
        จนถึงวันที่ <span class="dotted" style="width: 140px;">{{ toThaiNum(\Carbon\Carbon::parse($leaveRequest->end_date)->day) }}</span> 
        เดือน <span class="dotted" style="width: 170px;">{{ \Carbon\Carbon::parse($leaveRequest->end_date)->locale('th')->monthName }}</span> 
        พ.ศ. <span class="dotted" style="width: 140px;">{{ toThaiNum(\Carbon\Carbon::parse($leaveRequest->end_date)->year + 543) }}</span>
    </div>

       <div class="content-line">
        ระหว่างลานี้ กระผม จะไปที่ <span class="dotted" style="width: 430px;">{{ is_array($leaveRequest->contact_address) ? toThaiNum($leaveRequest->contact_address['house'] ?? '-') : '-' }}</span>
        </div>

<div class="content-line">
        จังหวัด<span class="dotted" style="width: 140px;">{{ is_array($leaveRequest->contact_address) ? ($leaveRequest->contact_address['province'] ?? '-') : '-' }}</span>
        ในวันที่<span class="dotted" style="width: 80px;">{{ toThaiNum(\Carbon\Carbon::parse($leaveRequest->start_date)->day) }}</span>
        เดือน<span class="dotted" style="width: 120px;">{{ \Carbon\Carbon::parse($leaveRequest->start_date)->locale('th')->monthName }}</span>
        พ.ศ.<span class="dotted" style="width: 80px;">{{ toThaiNum(\Carbon\Carbon::parse($leaveRequest->start_date)->year + 543) }}</span>
        </div>


        <div class="content-line">
        และจะกลับในวันที่<span class="dotted" style="width: 140px;">{{ toThaiNum(\Carbon\Carbon::parse($leaveRequest->end_date)->day) }}</span>
        เดือน<span class="dotted" style="width: 120px;">{{ \Carbon\Carbon::parse($leaveRequest->end_date)->locale('th')->monthName }}</span>
        พ.ศ.<span class="dotted" style="width: 150px;">{{ toThaiNum(\Carbon\Carbon::parse($leaveRequest->end_date)->year + 543) }}</span>
        </div>
...
if ($isApproved) {
    ...
    <div class="content-line indent-1">
        ...
        <div class="content-line indent-1">
        การลากิจครั้งนี้ อยู่ในอำนาจของ<span class="dotted" style="width: 200px;">{{ $approverHead ? ($approverHead->approver->position ?? $leaveRequest->user->supervisor->position ?? 'หัวหน้าแผนก') : ($leaveRequest->user->supervisor->position ?? 'หัวหน้าแผนก') }}</span>อนุญาตได้ตามข้อบังคับ
    </div>
        ...
        <div style="width: 48%; float: left; padding-right: 10px;">
            ...
            <div class="checker-section" style="margin-top: 20px;">
                <div style="text-align: center;">
                    <div style="margin-bottom: 5px;">
                        <div style="">
                            <div class="text-bold">- อนุญาตให้ลากิจได้</div>
                            <div style="margin-bottom: 5px;">
        ...
        <div style="width: 48%; float: right; padding-left: 10px;">
            ...
            <div style="">
                ...
                <div style="text-align: center; white-space: nowrap;">
                     ...
                     @elseif($approverManager)
                         {{-- Manager acknowledged but no signature (รับทราบ stage) --}}
                         <div style="margin-top: 10px;">
                             <span style="border-bottom: 1px dotted #000; padding: 0 20px;">{{ $approverManager->approver->name }}</span>
                         </div>

    ...
    <div x-data="{ showPdf: false, pdfUrl: null }" class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8 pb-32">
        ...
        <div class="flex flex-col lg:flex-row lg:items-end justify-between mb-16 gap-8 relative">
            <div class="relative">
                ...
                <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-100 text-slate-500 text-sm font-bold uppercase tracking-[0.2em] mb-4 border border-slate-200 shadow-sm">
            ...
            <a href="{{ route('leave-request.create') }}" 
               class="group inline-flex items-center justify-center px-10 py-5 bg-slate-900 hover:bg-brand-600 text-white font-bold text-xl rounded-[2.5rem] shadow-2xl hover:shadow-brand-500/40 transition-all duration-300 transform hover:-translate-y-2 active:scale-95">
        ...
        @if (session('status'))
            <div x-data="{ show: true }" x-show="show" x-transition:leave="transition ease-in duration-300" class="mb-10">
                <div class="bg-emerald-50 border border-emerald-100 rounded-[2.5rem] p-5 flex items-center gap-6 shadow-xl shadow-emerald-500/5 relative overflow-hidden group">
                    ...
                    <div class="flex-1">
                        <h4 class="font-bold text-emerald-900 tracking-tight text-lg">ดำเนินการเรียบร้อย</h4>
                        <p class="text-base font-semibold text-emerald-600/80">{{ session('status') }}</p>
                    </div>
                        ...
                        <div class="flex flex-col xl:flex-row gap-10 relative z-10">
                            ...
                            <div class="flex-1 min-w-0 flex flex-col justify-center py-2">
                                <div class="flex flex-wrap items-center gap-3 mb-6">
                                    ...
                                    @endphp
                                    <span class="inline-flex items-center px-5 py-2 rounded-2xl text-sm font-bold uppercase tracking-wider border shadow-sm {{ $typeStyle }}">
                                        <i data-lucide="{{ $typeIcon }}" class="w-4 h-4 mr-2.5"></i>
                                        {{ $req->leaveType->name }}
                                    </span>
                                    
                                    <span class="px-5 py-2 rounded-2xl bg-slate-900 text-white text-xs font-bold uppercase tracking-[0.2em] shadow-lg shadow-slate-900/20">
                                        <i data-lucide="timer" class="w-3.5 h-3.5 mr-2 inline text-brand-400"></i>{{ $req->total_days + 0 }} วัน
                                    </span>
                                    
                                    <span class="text-xs text-slate-300 font-bold ml-auto uppercase tracking-widest hidden sm:block">REQUEST ID: #{{ $req->id }}</span>
                                </div>
                                ...
                                <div class="flex flex-wrap items-center gap-6">
                                    <div class="flex items-center gap-3 text-base font-bold text-slate-400 bg-slate-50 px-5 py-2.5 rounded-2xl border border-slate-100">
                                    ...
                                    @if($req->attachment_path)
                                        <a href="{{ asset('storage/' . $req->attachment_path) }}" target="_blank" class="group/link flex items-center gap-3 text-sm font-bold text-brand-600 uppercase tracking-widest hover:text-brand-700">
                            ...
                            <div class="flex-shrink-0 w-full xl:w-64 flex flex-col justify-between items-end gap-6 pt-10 xl:pt-4 xl:pl-10 xl:border-l xl:border-slate-50">
                                ...
                                <div class="w-full">
                                    <label class="block text-right text-xs font-bold text-slate-300 mb-3 uppercase tracking-widest leading-none">สถานะปัจจุบัน</label>
                                    <div class="flex items-center gap-3 px-6 py-4 rounded-[1.5rem] font-bold w-full justify-center xl:justify-end shadow-2xl transition-transform hover:scale-105 {{ $statusConfig['bg'] }} {{ $statusConfig['text'] }} {{ $statusConfig['glow'] }}">
                                        <i data-lucide="{{ $statusConfig['icon'] }}" class="w-5 h-5"></i>
                                        <span class="text-base tracking-tight">{{ $statusConfig['label'] }}</span>
                                    </div>
                                </div>
                                
                                <div class="w-full flex flex-col gap-3">
                                    <button type="button" @click="showPdf = true; pdfUrl = '{{ route('leave-request.pdf', $req->id) }}'" class="group/pdf w-full inline-flex items-center justify-center gap-3 px-6 py-4 rounded-2xl bg-slate-50 text-slate-700 font-bold text-base hover:bg-slate-100 transition-all active:scale-95 border border-slate-100">
                                    ...
                                    @if(str_starts_with($req->status, 'pending_'))
                                        <form action="{{ route('leave-request.cancel', $req->id) }}" method="POST" onsubmit="return confirm('ยืนยันการยกเลิกคำขอลา? เมื่อยกเลิกแล้วจะไม่สามารถย้อนคืนได้');" class="w-full">
                                            @csrf
                                            @method('PUT')
                                            <button type="submit" class="w-full inline-flex items-center justify-center gap-3 px-6 py-4 rounded-2xl bg-white border border-rose-100 text-rose-500 font-bold text-base hover:bg-rose-50 hover:border-rose-200 transition-all active:scale-95 group/cancel">
                ...
                <div x-show="showPdf"
                    ...
                    <div class="bg-slate-50 px-8 py-6 flex flex-col sm:flex-row justify-between items-center gap-4 border-t border-slate-100">
                        ...
                        <div class="flex gap-4 w-full sm:w-auto">
                            <a :href="pdfUrl" target="_blank" class="flex-1 sm:flex-initial inline-flex items-center justify-center gap-3 px-8 py-3.5 bg-slate-900 text-white font-bold rounded-2xl shadow-xl hover:bg-black transition-all active:scale-95 text-base uppercase tracking-wider">
                                <i data-lucide="external-link" class="w-5 h-5"></i> 
                                เปิดแบบเต็มจอ
                            </a>
                            <button type="button" class="flex-1 sm:flex-initial px-8 py-3.5 bg-white text-slate-700 font-bold rounded-2xl shadow-md border border-slate-200 hover:bg-slate-50 transition-all active:scale-95 text-base uppercase tracking-wider" @click="showPdf = false">
                                ปิดหน้าต่าง
                            </button>
                        </div>
                    </div>

    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            <div class="px-8 py-6 border-b border-slate-50 bg-slate-50/50 flex items-center justify-between">
                <h3 class="font-bold text-slate-800 text-lg">แก้ไขข้อมูล: {{ $employee->name }}</h3>
                
                {{-- Delete Button (Right Aligned) --}}
                <form action="{{ route('employees.destroy', $employee->id) }}" method="POST" onsubmit="return confirm('ยืนยันการลบพนักงานคนนี้? การกระทำนี้ไม่สามารถย้อนกลับได้');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="text-red-500 hover:text-red-700 text-sm font-medium px-3 py-1.5 rounded-lg hover:bg-red-50 transition-colors">
                        <i data-lucide="trash-2" class="w-4 h-4 mr-1"></i> ลบพนักงาน
                    </button>
                </form>
            </div>

            <form action="{{ route('employees.update', $employee->id) }}" method="POST" class="p-8 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $employee->name) }}" class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email', $employee->email) }}" class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
                    ...
                    <div>
                        ...
                        <input type="text" name="position" value="{{ old('position', $employee->position) }}" class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="role" required class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
                            ...
                            <option value="deputy_director" {{ old('role', $employee->role) == 'deputy_director' ? 'selected' : '' }}>รองผู้อำนวยการโรงเรียนพลาธิการ กรมพลาธิการทหารเรือ</option>
                            <option value="director" {{ old('role', $employee->role) == 'director' ? 'selected' : '' }}>ผู้อำนวยการโรงเรียนพลาธิการ กรมพลาธิการทหารเรือ</option>
                            <option value="admin" {{ old('role', $employee->role) == 'admin' ? 'selected' : '' }}>ผู้ดูแลระบบ (Admin)</option>
                        </select>
                    </div>
                    ...
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">วันที่เริ่มงาน</label>
                         <input type="date" name="start_date" value="{{ old('start_date', $employee->start_date?->format('Y-m-d')) }}" class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
                    </div>

                    <!-- Vacation Leave Quota -->
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">สิทธิ์วันลาพักผ่อน (วัน/ปี)</label>
                        <input type="number" step="0.5" name="vacation_leave_days" value="{{ old('vacation_leave_days', $currentVacationQuota ?? 10) }}" class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
                        <p class="text-xs text-slate-400 mt-1">สามารถปรับลดหรือเพิ่มสิทธิ์วันลาได้ที่นี่ (ยอดคงเหลือจะถูกปรับอัตโนมัติ)</p>
                    </div>

                    <!-- Password Reset -->
                    <div class="md:col-span-2 border-t border-slate-100 pt-6 mt-2">
                        <label class="block text-sm font-bold text-slate-700 mb-2">เปลี่ยนรหัสผ่าน (เว้นว่างหากไม่ต้องการเปลี่ยน)</label>
                        <input type="password" name="password" class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500" placeholder="ตั้งรหัสผ่านใหม่...">
                    </div>
                </div>

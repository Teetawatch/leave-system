    ...
    <div class="max-w-[85rem] mx-auto py-10 px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-6 relative">
            <div class="relative z-10 p-2">
                <div class="absolute -left-10 -top-10 w-32 h-32 bg-rose-400 rounded-full blur-3xl opacity-20 pointer-events-none"></div>
                <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight flex items-center gap-3">
                    <span class="w-12 h-12 rounded-2xl bg-gradient-to-br from-rose-400 to-rose-600 text-white flex items-center justify-center shadow-lg shadow-rose-500/30">
                        <i data-lucide="crown" class="w-4 h-4"></i>
                    </span>
                    คำขอเปลี่ยนยามรอ ผอ. อนุมัติ
                </h1>
                <p class="text-slate-500 mt-2 text-lg pl-1">อนุมัติคำขอเปลี่ยนยามที่ผ่านการอนุมัติจาก รอง ผอ. แล้ว</p>
            </div>
            
             <div class="flex items-center gap-2">
                <span class="px-4 py-2 bg-white rounded-xl text-slate-500 text-sm font-bold border border-slate-200 shadow-sm">
                    <i data-lucide="list-checks" class="w-4 h-4 mr-2 text-rose-600"></i>
             ...
             <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-20 text-center relative overflow-hidden group">
                ...
                <div class="relative z-10">
                    <div class="w-32 h-32 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-8 group-hover:scale-110 transition-transform duration-500 border-4 border-white shadow-inner">
                        <i data-lucide="crown" class="w-4 h-4 text-slate-300 text-6xl group-hover:text-rose-400 transition-colors"></i>
                        <div class="absolute bottom-1 right-1 w-10 h-10 bg-rose-500 rounded-full border-4 border-white flex items-center justify-center shadow-lg transform translate-y-2 translate-x-2">
        ...
        @else
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 md:p-8 hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300 hover:-translate-y-1 relative group overflow-hidden"
                    ...
                    <!-- Top Decor -->
                    <div class="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-rose-50 to-rose-100 rounded-bl-[4rem] -mr-10 -mt-10 opacity-50 group-hover:scale-125 transition-transform duration-500"></div>
                    ...
                    <div class="flex flex-col lg:flex-row gap-8 relative z-10">
                        ...
                        <div class="flex-shrink-0 flex flex-row lg:flex-col items-center gap-6 lg:w-40 border-b lg:border-b-0 lg:border-r border-slate-50 pb-6 lg:pb-0 lg:pr-8">
                             <!-- Date Ticket -->
                            <div class="flex flex-col items-center justify-center w-24 h-28 bg-rose-50 rounded-2xl p-2 border border-rose-100 group-hover:bg-rose-100/50 transition-colors shadow-sm">
                                <span class="text-xs text-rose-600 font-bold uppercase tracking-wider mb-1">{{ \Carbon\Carbon::parse($req->duty_date)->locale('th')->isoFormat('MMM') }}</span>
                        ...
                        <div class="flex-1 min-w-0 flex flex-col justify-center">
                            <div class="flex flex-wrap items-center gap-3 mb-5">
                                <span class="inline-flex items-center px-3 py-1.5 rounded-xl text-sm font-bold shadow-sm bg-rose-50 text-rose-600 ring-1 ring-rose-100">
                                    <i data-lucide="shield" class="w-4 h-4 mr-2"></i>
                                    {{ $dutyPositions[$req->duty_position] ?? $req->duty_position }}
                                </span>
                                
                                <span class="flex items-center px-3 py-1.5 rounded-xl bg-purple-50 text-purple-600 text-xs font-bold border border-purple-100">
                                    <i data-lucide="stamp" class="w-4 h-4 mr-1.5"></i>
                                    รอง ผอ. อนุมัติแล้ว
                                </span>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4 mb-4">
                                <div class="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                    <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">ผู้รับมอบหมาย</p>
                                    <div class="flex items-center gap-2">
                                        <div class="h-8 w-8 rounded-lg bg-emerald-100 text-emerald-600 flex items-center justify-center font-bold text-xs">
                                            <i data-lucide="check" class="w-4 h-4"></i>
                                        </div>
                                        <p class="font-bold text-slate-800 text-sm">{{ $req->replacementUser->rank }} {{ $req->replacementUser->name }}</p>
                                    </div>
                                </div>
                                <div class="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                    <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">รอง ผอ. ที่อนุมัติ</p>
                                    <div class="flex items-center gap-2">
                                        <div class="h-8 w-8 rounded-lg bg-purple-100 text-purple-600 flex items-center justify-center font-bold text-xs">
                                            <i data-lucide="stamp" class="w-4 h-4"></i>
                                        </div>
                                        @if($req->directorApprover)
                                            <p class="font-bold text-slate-800 text-sm">{{ $req->directorApprover->rank }} {{ $req->directorApprover->name }}</p>
                                        @else
                        ...
                        <div class="flex-shrink-0 w-full lg:w-48 flex flex-col justify-center items-center gap-3 pt-6 lg:pt-0 border-t lg:border-t-0 lg:border-l border-slate-50 lg:pl-8">
                            <button @click="openApprove = true" class="w-full group flex items-center justify-center gap-3 px-4 py-4 bg-rose-500 hover:bg-rose-600 text-white rounded-2xl shadow-lg shadow-rose-500/20 transition-all hover:-translate-y-1 font-bold">
                                <i data-lucide="crown" class="w-4 h-4 text-lg group-hover:scale-110 transition-transform"></i>
                                อนุมัติ
                            </button>
                        </div>
                    </div>
                    ...
                    <div x-show="openApprove" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;"
                        ...
                        <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:p-0">
                            ...
                            <div class="bg-white rounded-[2rem] text-left overflow-hidden shadow-2xl transform transition-all relative"
                                ...
                                <form action="{{ route('guard-change.final-approve', $req->id) }}" method="POST" style="height: 100%; display: flex; flex-direction: column; overflow-y: auto;">
                                    @csrf
                                    ...
                                    <div class="bg-white p-8">
                                        <div class="mx-auto flex items-center justify-center h-20 w-20 rounded-full bg-rose-50 text-rose-500 mb-6 shadow-inner">
                                            <i data-lucide="crown" class="w-4 h-4 text-4xl"></i>
                                        </div>
                                        <div class="text-center">
                                            <h3 class="text-2xl font-black text-slate-900 tracking-tight">อนุมัติคำขอเปลี่ยนยาม (ขั้นสุดท้าย)</h3>
                                            ...
                                            <div class="mt-6">
                                                ...
                                                <div class="flex items-center justify-center gap-4 mb-4 bg-slate-50 p-3 rounded-xl border border-slate-100">
                                                    <label class="inline-flex items-center cursor-pointer gap-2">
                                                        <input type="radio" value="0" name="use_saved_radios_{{ $req->id }}" @click="useSaved = false; $nextTick(() => { resizeCanvas(); })" :checked="!useSaved" class="form-radio text-rose-500 focus:ring-rose-500 border-slate-300">
                                                        <span class="text-sm font-bold text-slate-700">วาดลายเซ็น</span>
                                                    </label>
                                                    <div class="w-px h-6 bg-slate-300"></div>
                                                    <label class="inline-flex items-center cursor-pointer gap-2">
                                                        <input type="radio" value="1" name="use_saved_radios_{{ $req->id }}" @click="useSaved = true" :checked="useSaved" class="form-radio text-rose-500 focus:ring-rose-500 border-slate-300">
                                                ...
                                                <div x-show="!useSaved" class="text-left">
                                                    ...
                                                    <div class="border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50 relative overflow-hidden h-48 hover:border-rose-300 hover:bg-rose-50/20 transition-all cursor-crosshair">
                                                        <canvas id="signature-canvas-{{ $req->id }}" class="w-full h-full"></canvas>
                                                    </div>
                                                    <p class="text-[10px] text-slate-400 mt-2 font-medium text-center">กรุณาเซ็นชื่อในกรอบด้านบน</p>
                                                </div>
                                                ...
                                                <div x-show="useSaved" style="display: none;" class="text-left">
                                                    ...
                                                    <div class="border-2 border-solid border-rose-100 rounded-2xl bg-rose-50/30 p-4 h-48 flex items-center justify-center relative overflow-hidden">
                                                        <img src="{{ asset('storage/' . Auth::user()->signature) }}" class="max-h-full max-w-full object-contain">
                                                        <div class="absolute top-2 right-2 bg-rose-100 text-rose-600 px-2 py-0.5 rounded text-[10px] font-bold">
                                            ...
                                            <div class="mt-6 text-left">
                                                ...
                                                <textarea name="comment" rows="2" class="block w-full rounded-2xl border-slate-200 bg-slate-50 focus:bg-white focus:border-rose-500 focus:ring-4 focus:ring-rose-500/10 transition-all text-sm font-medium" placeholder="ระบุความคิดเห็นเพิ่มเติม..."></textarea>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="bg-slate-50 px-8 py-6 flex flex-col-reverse sm:flex-row sm:justify-end gap-3 border-t border-slate-100">
                                        ...
                                        <button type="button" @click="submitForm($event)" class="w-full sm:w-auto inline-flex justify-center items-center px-6 py-3 rounded-xl border border-transparent shadow-lg shadow-rose-500/30 bg-rose-500 text-white font-bold hover:bg-rose-600 focus:outline-none transform hover:-translate-y-0.5 transition-all">
                                            <i data-lucide="crown" class="w-4 h-4 mr-2"></i> ยืนยันอนุมัติ
                                        </button>
                                    </div>
                                </form>
                            </div>

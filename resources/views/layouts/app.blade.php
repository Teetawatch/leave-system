...
<body class="font-sans antialiased bg-slate-50 text-slate-600" x-data="{ sidebarOpen: false, mobileProfileOpen: false, mobileNotifOpen: false }">
    ...
    <div class="md:hidden sticky top-0 z-20">
        <!-- Main Mobile Bar -->
        <div class="flex items-center justify-between bg-white/95 backdrop-blur-lg shadow-sm px-4 py-3 border-b border-slate-100">
            <!-- Left: Menu + Logo -->
            <div class="flex items-center gap-3">
                <button @click="sidebarOpen = !sidebarOpen" class="w-10 h-10 rounded-xl bg-slate-100 hover:bg-brand-50 text-slate-500 hover:text-brand-600 flex items-center justify-center transition-all active:scale-95">
                    <i data-lucide="menu" class="w-5 h-5"></i>
                </button>
                <div class="flex items-center gap-2">
                    <img src="{{ asset('images/logonavy.png') }}" alt="Logo" class="w-10 h-10 sm:w-12 sm:h-12 object-contain flex-shrink-0">
                    <span class="font-bold text-xs sm:text-sm text-slate-800 hidden xs:block truncate max-w-[150px] sm:max-w-none">ระบบบริหารจัดการงานธุรการด้านกำลังพล</span>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open" class="w-10 h-10 rounded-xl bg-slate-100 hover:bg-amber-50 text-slate-500 hover:text-amber-600 flex items-center justify-center transition-all active:scale-95 relative">
                        <i data-lucide="bell" class="w-5 h-5"></i>
                        @if(($navNotificationCount ?? 0) > 0)
                            <span class="absolute top-1 right-1 h-5 w-5 rounded-full bg-red-500 text-white text-[10px] font-bold flex items-center justify-center ring-2 ring-white">
                        ...
                        <div class="px-4 py-3 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                            ...
                            @if(($navNotificationCount ?? 0) > 0)
                                <form action="{{ route('notifications.markRead') }}" method="POST">
                                    @csrf
                                    <button type="submit" class="text-xs text-brand-600 hover:text-brand-700 font-medium">อ่านทั้งหมด</button>
                                </form>
                            @endif
                        </div>
                        <div class="max-h-64 overflow-y-auto">
                            @forelse(Auth::user()->unreadNotifications->take(5) as $notification)
                                <div class="px-4 py-3 border-b border-slate-50 hover:bg-slate-50">
                                    <p class="text-sm font-bold text-slate-800">{{ $notification->data['status'] === 'approved' ? 'อนุมัติแล้ว' : ($notification->data['status'] === 'pending' ? 'รอการอนุมัติ' : 'ถูกปฏิเสธ') }}</p>
                                    <p class="text-xs text-slate-500 mt-0.5 line-clamp-1">{{ $notification->data['message'] }}</p>
                                </div>
                            @empty
                ...
                <!-- User Avatar/Profile -->
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open" class="w-10 h-10 rounded-xl bg-gradient-to-br from-brand-500 to-brand-600 p-0.5 shadow-md shadow-brand-500/20 active:scale-95 transition-all overflow-hidden">
                    ...
                    <div x-show="open" @click.away="open = false"
                        ...
                        <div class="px-4 py-3 border-b border-slate-50">
                            <div class="flex items-center gap-3">
                                <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-brand-500 to-brand-600 p-0.5 shadow-md">
                                    <div class="w-full h-full rounded-lg bg-white flex items-center justify-center overflow-hidden">
                                        @if(Auth::user()->avatar)
                                            <img src="{{ asset('storage/' . Auth::user()->avatar) }}" alt="Avatar" class="w-full h-full object-cover">
                                        @else
                                            <span class="font-bold text-brand-600">{{ substr(Auth::user()->name, 0, 1) }}</span>
                                        @endif
                                    </div>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <p class="font-bold text-slate-800 text-sm truncate">{{ Auth::user()->rank }} {{ Auth::user()->name }}</p>
                                    <p class="text-xs text-slate-500 truncate">{{ Auth::user()->department ?? 'Staff' }}</p>
                                </div>
                            </div>
                        </div>
                        
                        <a href="{{ route('profile.edit') }}" class="flex items-center px-4 py-3 text-sm text-slate-600 hover:bg-slate-50 hover:text-brand-600 transition-colors">
                            <i data-lucide="user" class="w-4 h-4 mr-3"></i> แก้ไขโปรไฟล์
                        </a>
                        
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="flex w-full items-center px-4 py-3 text-sm text-red-500 hover:bg-red-50 transition-colors">
    ...
    <div class="flex min-h-screen">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" 
            ...
            <div class="h-auto py-5 flex flex-col items-center px-4 border-b border-slate-50 text-center">
                ...
                <div>
                    <h1 class="text-sm font-bold text-slate-800 tracking-tight leading-tight mb-1">ระบบบริหารจัดการงานธุรการด้านกำลังพล</h1>
                    <p class="text-[10px] text-slate-400 uppercase tracking-wider font-semibold">โรงเรียนพลาธิการ กรมพลาธิการทหารเรือ</p>
                </div>
            </div>
            ...
            <nav class="flex-1 px-3 space-y-1 overflow-y-auto custom-scrollbar py-4" x-data="{ 
                ...
                <div class="pt-3">
                    <button @click="openMenus.leave = !openMenus.leave" 
                        ...
                        <i data-lucide="chevron-down" class="w-3 h-3 text-slate-400 transition-transform duration-200" :class="openMenus.leave && 'rotate-180'"></i>
                    </button>
                    <div x-show="openMenus.leave" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-2" class="mt-1 ml-3 pl-3 border-l-2 border-slate-100 space-y-0.5">
                        <a href="{{ route('leave-request.create') }}" 
                           class="flex items-center px-3 py-2 rounded-lg transition-all duration-200 group {{ request()->routeIs('leave-request.create') ? 'bg-brand-50 text-brand-600' : 'text-slate-500 hover:bg-slate-50 hover:text-brand-600' }}">
                            <i data-lucide="send" class="w-3.5 h-3.5 {{ request()->routeIs('leave-request.create') ? 'text-brand-600' : 'text-slate-400 group-hover:text-brand-600' }}"></i>
                        ...
                        <a href="{{ route('leave-request.index') }}" 
                           class="flex items-center px-3 py-2 rounded-lg transition-all duration-200 group {{ request()->routeIs('leave-request.index') ? 'bg-brand-50 text-brand-600' : 'text-slate-500 hover:bg-slate-50 hover:text-brand-600' }}">
                            <i data-lucide="history" class="w-3.5 h-3.5 {{ request()->routeIs('leave-request.index') ? 'text-brand-600' : 'text-slate-400 group-hover:text-brand-600' }}"></i>
                    ...
                    <button @click="openMenus.guard = !openMenus.guard" 
                        ...
                        <div class="flex items-center">
                            ...
                            @if(isset($navGuardChangePendingMe) && $navGuardChangePendingMe > 0)
                                <span class="ml-2 bg-red-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full">{{ $navGuardChangePendingMe }}</span>
                            @endif
                        </div>
                        <i data-lucide="chevron-down" class="w-3 h-3 text-slate-400 transition-transform duration-200" :class="openMenus.guard && 'rotate-180'"></i>
                    </button>
                    <div x-show="openMenus.guard" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-2" class="mt-1 ml-3 pl-3 border-l-2 border-slate-100 space-y-0.5">
                        <a href="{{ route('guard-change.create') }}" 
                           class="flex items-center px-3 py-2 rounded-lg transition-all duration-200 group {{ request()->routeIs('guard-change.create') ? 'bg-emerald-50 text-emerald-600' : 'text-slate-500 hover:bg-slate-50 hover:text-emerald-600' }}">
                            <i data-lucide="plus" class="w-3.5 h-3.5 {{ request()->routeIs('guard-change.create') ? 'text-emerald-600' : 'text-slate-400 group-hover:text-emerald-600' }}"></i>
                        ...
                        <a href="{{ route('guard-change.index') }}" 
                           class="flex items-center px-3 py-2 rounded-lg transition-all duration-200 group {{ request()->routeIs('guard-change.index') || request()->routeIs('guard-change.show') ? 'bg-emerald-50 text-emerald-600' : 'text-slate-500 hover:bg-slate-50 hover:text-emerald-600' }}">
                            <i data-lucide="list" class="w-3.5 h-3.5 {{ request()->routeIs('guard-change.index') || request()->routeIs('guard-change.show') ? 'text-emerald-600' : 'text-slate-400 group-hover:text-emerald-600' }}"></i>
                        ...
                        <a href="{{ route('guard-change.approvals') }}" 
                           class="flex items-center px-3 py-2 rounded-lg transition-all duration-200 group {{ request()->routeIs('guard-change.approvals') ? 'bg-amber-50 text-amber-600' : 'text-slate-500 hover:bg-slate-50 hover:text-amber-600' }}">
                            <i data-lucide="user-check" class="w-3.5 h-3.5 {{ request()->routeIs('guard-change.approvals') ? 'text-amber-600' : 'text-slate-400 group-hover:text-amber-600' }}"></i>
                            <span class="ml-2.5 text-sm font-medium flex-1">คำขอหาฉัน</span>
                            @if(isset($navGuardChangePendingMe) && $navGuardChangePendingMe > 0)
                                <span class="bg-red-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full">{{ $navGuardChangePendingMe }}</span>
                            @endif
                        </a>
                    </div>
                </div>
                ...
                <div class="pt-2">
                    <button @click="openMenus.approval = !openMenus.approval" 
                        ...
                        <div class="flex items-center">
                            ...
                            @endphp
                            @if($totalPending > 0)
                                <span class="ml-2 bg-red-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full">{{ $totalPending }}</span>
                            @endif
                        </div>
                        <i data-lucide="chevron-down" class="w-3 h-3 text-slate-400 transition-transform duration-200" :class="openMenus.approval && 'rotate-180'"></i>
                    </button>
                    <div x-show="openMenus.approval" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-2" class="mt-1 ml-3 pl-3 border-l-2 border-slate-100 space-y-0.5">
                        <a href="{{ route('approvals.index') }}" 
                           class="flex items-center px-3 py-2 rounded-lg transition-all duration-200 group {{ request()->routeIs('approvals.index') ? 'bg-brand-50 text-brand-600' : 'text-slate-500 hover:bg-slate-50 hover:text-brand-600' }}">
                            <i data-lucide="file-pen" class="w-3.5 h-3.5 {{ request()->routeIs('approvals.index') ? 'text-brand-600' : 'text-slate-400 group-hover:text-brand-600' }}"></i>
                            <span class="ml-2.5 text-sm font-medium flex-1">อนุมัติใบลา</span>
                            @if(isset($navPendingCount) && $navPendingCount > 0)
                                <span class="bg-red-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full">{{ $navPendingCount }}</span>
                            @endif
                        </a>
                        ...
                        <a href="{{ route('guard-change.director-approvals') }}" 
                           class="flex items-center px-3 py-2 rounded-lg transition-all duration-200 group {{ request()->routeIs('guard-change.director-approvals') ? 'bg-purple-50 text-purple-600' : 'text-slate-500 hover:bg-slate-50 hover:text-purple-600' }}">
                            <i data-lucide="stamp" class="w-3.5 h-3.5 {{ request()->routeIs('guard-change.director-approvals') ? 'text-purple-600' : 'text-slate-400 group-hover:text-purple-600' }}"></i>
                            ...
                            @if(isset($navGuardChangeDeputyCount) && $navGuardChangeDeputyCount > 0)
                                <span class="bg-red-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full">{{ $navGuardChangeDeputyCount }}</span>
                            @endif
                        </a>
                        @endif
                        ...
                        <a href="{{ route('guard-change.final-approvals') }}" 
                           class="flex items-center px-3 py-2 rounded-lg transition-all duration-200 group {{ request()->routeIs('guard-change.final-approvals') ? 'bg-rose-50 text-rose-600' : 'text-slate-500 hover:bg-slate-50 hover:text-rose-600' }}">
                            <i data-lucide="crown" class="w-3.5 h-3.5 {{ request()->routeIs('guard-change.final-approvals') ? 'text-rose-600' : 'text-slate-400 group-hover:text-rose-600' }}"></i>
                            <span class="ml-2.5 text-sm font-medium flex-1">ผอ. อนุมัติ</span>
                            @if(isset($navGuardChangeFinalCount) && $navGuardChangeFinalCount > 0)
                                <span class="bg-red-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full">{{ $navGuardChangeFinalCount }}</span>
                            @endif
                        </a>
                        @endif
                        ...
                        <div class="pt-1 mt-1 border-t border-slate-100">
                            <a href="{{ route('reports.index') }}" 
                               class="flex items-center px-3 py-2 rounded-lg transition-all duration-200 group {{ request()->routeIs('reports.index') ? 'bg-brand-50 text-brand-600' : 'text-slate-500 hover:bg-slate-50 hover:text-brand-600' }}">
                                <i data-lucide="pie-chart" class="w-3.5 h-3.5 {{ request()->routeIs('reports.index') ? 'text-brand-600' : 'text-slate-400 group-hover:text-brand-600' }}"></i>
                            ...
                            <a href="{{ route('reports.guard-change') }}" 
                               class="flex items-center px-3 py-2 rounded-lg transition-all duration-200 group {{ request()->routeIs('reports.guard-change') ? 'bg-indigo-50 text-indigo-600' : 'text-slate-500 hover:bg-slate-50 hover:text-indigo-600' }}">
                                <i data-lucide="repeat" class="w-3.5 h-3.5 {{ request()->routeIs('reports.guard-change') ? 'text-indigo-600' : 'text-slate-400 group-hover:text-indigo-600' }}"></i>
                            ...
                            <a href="{{ route('attendance-reports.index') }}" 
                               class="flex items-center px-3 py-2 rounded-lg transition-all duration-200 group {{ request()->routeIs('attendance-reports.*') ? 'bg-teal-50 text-teal-600' : 'text-slate-500 hover:bg-slate-50 hover:text-teal-600' }}">
                                <i data-lucide="scan-face" class="w-3.5 h-3.5 {{ request()->routeIs('attendance-reports.*') ? 'text-teal-600' : 'text-slate-400 group-hover:text-teal-600' }}"></i>
                ...
                <div class="pt-2">
                    <button @click="openMenus.admin = !openMenus.admin" 
                        ...
                        <i data-lucide="chevron-down" class="w-3 h-3 text-slate-400 transition-transform duration-200" :class="openMenus.admin && 'rotate-180'"></i>
                    </button>
                    <div x-show="openMenus.admin" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-2" class="mt-1 ml-3 pl-3 border-l-2 border-slate-100 space-y-0.5">
                        <a href="{{ route('employees.index') }}" 
                           class="flex items-center px-3 py-2 rounded-lg transition-all duration-200 group {{ request()->routeIs('employees.*') ? 'bg-brand-50 text-brand-600' : 'text-slate-500 hover:bg-slate-50 hover:text-brand-600' }}">
                            <i data-lucide="users" class="w-3.5 h-3.5 {{ request()->routeIs('employees.*') ? 'text-brand-600' : 'text-slate-400 group-hover:text-brand-600' }}"></i>
                        ...
                        <a href="{{ route('departments.index') }}" 
                           class="flex items-center px-3 py-2 rounded-lg transition-all duration-200 group {{ request()->routeIs('departments.*') ? 'bg-brand-50 text-brand-600' : 'text-slate-500 hover:bg-slate-50 hover:text-brand-600' }}">
                            <i data-lucide="network" class="w-3.5 h-3.5 {{ request()->routeIs('departments.*') ? 'text-brand-600' : 'text-slate-400 group-hover:text-brand-600' }}"></i>
                        ...
                        <a href="{{ route('settings.index') }}" 
                           class="flex items-center px-3 py-2 rounded-lg transition-all duration-200 group {{ request()->routeIs('settings.*') ? 'bg-brand-50 text-brand-600' : 'text-slate-500 hover:bg-slate-50 hover:text-brand-600' }}">
                            <i data-lucide="settings-2" class="w-3.5 h-3.5 {{ request()->routeIs('settings.*') ? 'text-brand-600' : 'text-slate-400 group-hover:text-brand-600' }}"></i>
                        ...
                        <a href="{{ route('leave-entitlements.index') }}" 
                           class="flex items-center px-3 py-2 rounded-lg transition-all duration-200 group {{ request()->routeIs('leave-entitlements.*') ? 'bg-emerald-50 text-emerald-600' : 'text-slate-500 hover:bg-slate-50 hover:text-emerald-600' }}">
                            <i data-lucide="calendar-plus" class="w-3.5 h-3.5 {{ request()->routeIs('leave-entitlements.*') ? 'text-emerald-600' : 'text-slate-400 group-hover:text-emerald-600' }}"></i>
            ...
            <div class="p-4 border-t border-slate-50">
                 <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="flex w-full items-center px-4 py-3 rounded-xl text-slate-400 hover:bg-slate-50 hover:text-red-500 transition-all duration-200 group">
        ...
        <main class="flex-1 flex flex-col min-w-0 bg-slate-50/50">
            <!-- Topbar (Desktop) -->
            <header class="hidden md:flex items-center justify-between h-20 bg-white border-b border-slate-100 px-8 sticky top-0 z-50 relative">
                <div>
                ...
                <div class="flex items-center gap-6">
                    ...
                    <!-- Notifications -->
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open" @click.away="open = false" class="relative p-2 text-slate-400 hover:text-brand-600 transition-colors rounded-full hover:bg-slate-50 focus:outline-none">
                            <i data-lucide="bell" class="w-5 h-5"></i>
                            @if(($navNotificationCount ?? 0) > 0)
                        ...
                        <div x-show="open" 
                            ...
                            <div class="px-4 py-3 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                                ...
                                @if(($navNotificationCount ?? 0) > 0)
                                    <form action="{{ route('notifications.markRead') }}" method="POST">
                                        @csrf
                                        <button type="submit" class="text-xs text-brand-600 hover:text-brand-700 font-medium hover:underline">
                                            อ่านทั้งหมด
                                        </button>
                                    </form>
                                @endif
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open" @click.away="open = false" class="flex items-center gap-3 focus:outline-none group">
                            <div class="text-right hidden md:block">
                                <p class="text-sm font-bold text-slate-700 group-hover:text-brand-600 transition-colors">{{ Auth::user()->rank }} {{ Auth::user()->name }}</p>
                                <p class="text-xs text-slate-400">{{ Auth::user()->department ?? 'Staff' }}</p>
                            </div>
                            <div class="w-10 h-10 rounded-full bg-gradient-to-br from-brand-500 to-brand-600 p-0.5 shadow-md shadow-brand-500/20 group-hover:shadow-brand-500/40 transition-all overflow-hidden">
                            ...
                            <i data-lucide="chevron-down" class="w-3 h-3 text-slate-300 group-hover:text-brand-600 transition-colors"></i>
                        </button>
                        ...
                        <div x-show="open" 
                            ...
                            <a href="{{ route('profile.edit') }}" class="flex items-center px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50 hover:text-brand-600 transition-colors">
                                <i data-lucide="user" class="w-4 h-4 mr-2"></i> โปรไฟล์
                            </a>
                            
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="flex w-full items-center px-4 py-2.5 text-sm text-red-500 hover:bg-red-50 transition-colors">
